...
function normalizeAnswer({
  ...
  if (questionType === QuestionType.LIKERT_1_5) {
    if (!valueLikert) return 0;
    return clamp01((valueLikert - 1) / 4);
  }

  if (questionType === QuestionType.NUMBER) {
    const n = valueNumber ?? 0;
    if (dimension === UsabilityDimension.EFFICIENCY) {
...
export class EvaluationsService {
  ...
  private async assertCanManageEvaluation(user: JwtUser, evaluationId: number) {
    if (user.role === RoleName.ADMIN) return;
  ...
  private async assertCanSubmitEvaluationData(
    user: JwtUser,
    evaluationId: number,
  ) {
  ...
  async create(user: JwtUser, dto: CreateEvaluationDto) {
    if (user.role !== RoleName.ADMIN) {
      throw new ForbiddenException(
        'Solo el administrador puede crear evaluaciones',
      );
    }
    ...
    const where =
      user.role === RoleName.ADMIN
        ...
        : {
            ...
            status: { not: EvaluationStatus.DRAFT },
          };
  ...
  async deleteEvaluation(user: JwtUser, evaluationId: number) {
  ...
  async getById(user: JwtUser, evaluationId: number) {
  ...
  async updateScoringWeights(
    user: JwtUser,
    evaluationId: number,
    dto: { effectiveness?: number; efficiency?: number; satisfaction?: number },
  ) {
    await this.assertCanManageEvaluation(user, evaluationId);
    ...
    const next = {
      scoringWeightEffectiveness:
        dto.effectiveness ?? existing.scoringWeightEffectiveness,
      scoringWeightEfficiency:
        dto.efficiency ?? existing.scoringWeightEfficiency,
      scoringWeightSatisfaction:
        dto.satisfaction ?? existing.scoringWeightSatisfaction,
    };

    const changed =
      existing.scoringWeightEffectiveness !== next.scoringWeightEffectiveness ||
    ...
    const sum =
      next.scoringWeightEffectiveness +
      next.scoringWeightEfficiency +
      next.scoringWeightSatisfaction;

    if (sum <= 0) {
      throw new BadRequestException(
        'Al menos un peso de scoring debe ser mayor que 0',
      );
    }
  ...
  async getMySelection(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
  ...
  async selectInterface(
    ...
    if (!intf) throw new NotFoundException('Interfaz no encontrada');
    if (intf.evaluationId !== evaluationId) {
      throw new BadRequestException('Interfaz no pertenece a la evaluación');
    }
    ...
    if (existing && existing.interfaceId !== interfaceId) {
      throw new BadRequestException(
        'Ya seleccionaste una interfaz para esta evaluación',
      );
    }
  ...
  async getReport(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
    const meta = await this.prisma.evaluation.findUnique({
      where: { id: evaluationId },
      select: { createdById: true, status: true },
    });
    if (!meta) throw new NotFoundException('Evaluación no encontrada');

    const canSeeAllAttempts =
      user.role === RoleName.ADMIN || meta.createdById === user.userId;
  ...
  async getReportPdf(user: JwtUser, evaluationId: number) {
    ...
    const fmtUserType = (v: typeof report.evaluation.userType) =>
      v === 'NOVICE' ? 'Novato' : 'Experto';
    const fmtEvaluationStatus = (v: typeof report.evaluation.status) => {
  ...
  <div class="kpi">
    ...
    <div class="kpiValue">${escapeHtml(
      percent(report.result.effectivenessScore),
    )}</div>
  </div>
  <div class="kpi">
  ...
  <div class="kpi">
    ...
    <div class="kpiValue">${escapeHtml(
      percent(report.result.satisfactionScore),
    )}</div>
  </div>
  <div class="kpi">
    ...
    const winnerInterfaceHtml = (() => {
      ...
      const img =
        winnerInterface.imageUrl && winnerInterface.imageUrl.trim()
          ? `<div class="imgWrap"><img class="img" src="${escapeHtml(
              winnerInterface.imageUrl,
            )}" /></div>`
...
<body>
  ...
  <div class="muted small">Generado: ${escapeHtml(
  ...
  <section class="section">
    ...
    <div class="grid2">
      <div class="card">
        ...
        <div><span class="strong">Título:</span> ${escapeHtml(
          report.evaluation.title,
        )}</div>
        <div><span class="strong">Sistema:</span> ${escapeHtml(
          report.evaluation.systemName,
        )}</div>
        <div><span class="strong">Tipo de usuario:</span> ${escapeHtml(
          fmtUserType(report.evaluation.userType),
        )}</div>
        <div><span class="strong">Estado:</span> ${escapeHtml(
          fmtEvaluationStatus(report.evaluation.status),
        )}</div>
        <div class="pre">Efectividad: ${escapeHtml(
          String(report.evaluation.scoringWeightEffectiveness),
        )} · Eficiencia: ${escapeHtml(
          String(report.evaluation.scoringWeightEfficiency),
        )} · Satisfacción: ${escapeHtml(
          String(report.evaluation.scoringWeightSatisfaction),
        )}</div>
      </div>
      <div class="card">
  ...
  async addInterface(
    ...
    const prototypeUrl =
      dto.prototypeUrl && dto.prototypeUrl.trim()
  ...
  async addTask(user: JwtUser, interfaceId: number, dto: CreateTaskDto) {
    const intf = await this.prisma.interface.findUnique({
      where: { id: interfaceId },
      select: { evaluationId: true },
    });
    ...
    const nextImageUrl =
      dto.imageUrl === undefined
        ...
        : dto.imageUrl.trim()
          ? dto.imageUrl.trim()
          : null;
    const nextPrototypeUrl =
      dto.prototypeUrl === undefined
        ? undefined
        : dto.prototypeUrl.trim()
          ? dto.prototypeUrl.trim()
          : null;
  ...
  async deleteInterface(user: JwtUser, interfaceId: number) {
    const intf = await this.prisma.interface.findUnique({
      where: { id: interfaceId },
      select: { id: true, evaluationId: true },
    });
  ...
  async deleteTask(user: JwtUser, taskId: number) {
    const task = await this.prisma.task.findUnique({
      where: { id: taskId },
  ...
  async deleteQuestion(user: JwtUser, questionId: number) {
    const q = await this.prisma.question.findUnique({
      where: { id: questionId },
      select: { interface: { select: { evaluationId: true } } },
    });
  ...
  async submitAnswers(
    ...
    if (existingAnswersCount > 0) {
      throw new ConflictException(
        'Las respuestas para esta interfaz ya fueron guardadas y no se pueden modificar.',
      );
    }
    ...
    const rows = dto.answers.map((a) => {
      ...
      if (!q) {
        throw new BadRequestException('Pregunta inválida');
      }
      ...
      if (q.type === QuestionType.LIKERT_1_5 && valueLikert === null) {
        throw new BadRequestException('Respuesta Likert requerida');
      }
      if (q.type === QuestionType.NUMBER && valueNumber === null) {
        throw new BadRequestException('Respuesta numérica requerida');
      }
      if (q.type === QuestionType.BOOLEAN && valueBoolean === null) {
        throw new BadRequestException('Respuesta booleana requerida');
      }
      if (q.type === QuestionType.TEXT && valueText === null) {
        throw new BadRequestException('Respuesta de texto requerida');
      }
  ...
  async getMyAnswers(user: JwtUser, evaluationId: number, interfaceId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
  ...
  async submitTaskAttempt(
    ...
    if (task.interface.evaluationId !== evaluationId) {
      throw new BadRequestException('Tarea no pertenece a la evaluación');
    }
  ...
  async computeResults(user: JwtUser, evaluationId: number) {
    await this.assertCanManageEvaluation(user, evaluationId);
    ...
    if (answers.length === 0) {
      throw new BadRequestException(
        'Aún no hay respuestas registradas para calcular resultados',
      );
    }
    ...
    const computeForInterface = (interfaceId: number) => {
      const agg = new Map<
        UsabilityDimension,
        { sum: number; weightSum: number }
      >([
      ...
      for (const a of answersByInterface.get(interfaceId) ?? []) {
        const q = questionById.get(a.questionId);
        if (!q) continue;
        if (q.interfaceId !== interfaceId) continue;
      ...
      const effectivenessScore = clamp01(
        (agg.get(UsabilityDimension.EFFECTIVENESS)?.sum ?? 0) /
          (agg.get(UsabilityDimension.EFFECTIVENESS)?.weightSum ?? 1),
      );
      const efficiencyScore = clamp01(
        (agg.get(UsabilityDimension.EFFICIENCY)?.sum ?? 0) /
          (agg.get(UsabilityDimension.EFFICIENCY)?.weightSum ?? 1),
      );
      const satisfactionScore = clamp01(
        (agg.get(UsabilityDimension.SATISFACTION)?.sum ?? 0) /
          (agg.get(UsabilityDimension.SATISFACTION)?.weightSum ?? 1),
      );
    ...
    const winnerInterfaceName =
      interfaces.find((i) => i.id === winnerInterfaceId)?.name ??
    ...
    for (const a of answersByInterface.get(winnerInterfaceId) ?? []) {
      const q = questionById.get(a.questionId);
      if (!q) continue;
      if (q.interfaceId !== winnerInterfaceId) continue;
      const v = normalizeAnswer({
        questionType: q.type,
        dimension: q.dimension,
        valueNumber: a.valueNumber,
    ...
    const conclusionsLines: string[] = [];
    const recLines: string[] = [];

    const addDimensionText = (
      dim: UsabilityDimension,
      score: number,
      good: string,
      bad: string,
      recs: string[],
    ...
    addDimensionText(
      UsabilityDimension.EFFECTIVENESS,
      effectivenessScore,
      'Efectividad adecuada: los usuarios logran el objetivo con pocos problemas',
      'Efectividad baja: hay dificultad para lograr el objetivo o se presentan errores',
      [
    ...
    addDimensionText(
      ...
      efficiencyScore,
      'Eficiencia adecuada: el esfuerzo para lograr el objetivo es razonable',
      'Eficiencia baja: el flujo requiere demasiado esfuerzo, tiempo o pasos innecesarios',
      [
    ...
    addDimensionText(
      ...
      satisfactionScore,
      'Satisfacción adecuada: la percepción de uso es positiva',
      'Satisfacción baja: la experiencia genera frustración o falta de confianza',
      [
    ...
    const worstQuestionPrompts = (dim: UsabilityDimension, limit = 3) => {
    ...
    const stories: Array<{
      dimension: UsabilityDimension | null;
      title: string;
      narrative: string;
    ...
    if (effectivenessScore < 0.7) {
      stories.push({
        dimension: UsabilityDimension.EFFECTIVENESS,
        title: 'Prevención de errores y recuperación',
        narrative:
          'Como usuario, quiero que la interfaz sea clara y me ayude a evitar errores para lograr el objetivo sin bloqueos.',
        acceptanceCriteria: [
          '- Se validan entradas y se muestran mensajes accionables',
          '- Se reduce la ambigüedad en etiquetas y llamadas a la acción',
          ...worstQuestionPrompts(UsabilityDimension.EFFECTIVENESS).map(
            (p) => `- Revisar pregunta: ${p}`,
          ),
        ].join('\n'),
        priority: 1,
      });
    ...
    if (efficiencyScore < 0.7) {
      stories.push({
        dimension: UsabilityDimension.EFFICIENCY,
        title: 'Optimización del flujo',
        narrative:
          'Como usuario, quiero lograr el objetivo con menos pasos y en menos tiempo para ahorrar esfuerzo.',
        acceptanceCriteria: [
    ...
    if (satisfactionScore < 0.7) {
      stories.push({
        ...
        narrative:
          'Como usuario, quiero recibir feedback inmediato y una interfaz consistente para sentir confianza al interactuar.',
        acceptanceCriteria: [
          '- Indicadores de carga, éxito y error visibles',
          '- Consistencia visual en componentes y jerarquía de información',
          ...worstQuestionPrompts(UsabilityDimension.SATISFACTION).map(
            (p) => `- Revisar pregunta: ${p}`,
          ),
        ].join('\n'),
        priority: 3,
    ...
    if (stories.length === 0) {
      stories.push({
        ...
        narrative:
          'Como equipo, queremos mantener la usabilidad alcanzada y detectar regresiones en nuevas versiones.',
        acceptanceCriteria:
          '- Repetir evaluación con nuevas muestras de usuarios\n- Comparar resultados por versión\n- Registrar incidencias y mejoras priorizadas',
        priority: 3,
      });
    }
    ...
    const scoreForDim = (
      ...
      if (dim === UsabilityDimension.EFFECTIVENESS) return b.effectivenessScore;
      if (dim === UsabilityDimension.EFFICIENCY) return b.efficiencyScore;
      return b.satisfactionScore;
    };

    const bestInterfaceIdForDimension = (dim: UsabilityDimension) => {
  ...
  async listManualUserStories(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
  ...
  async createManualUserStory(
    ...
    const created = await this.prisma.manualUserStory.create({
      data: {
        ...
        mosCow: dto.mosCow ?? MoSCoWPriority.SHOULD,
  ...
  async updateManualUserStory(
    ...
    if (triesToEditLockedFields) {
      throw new ConflictException(
        'La historia ya fue registrada y no se pueden modificar sus campos. Solo se permite vincular interfaces.',
      );
    }
  ...
  async listEvaluators(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
  ...
  async assignEvaluatorByEmail(
    ...
    if (!evaluator) throw new NotFoundException('Usuario no encontrado');
  ...
  async startSession(user: JwtUser, evaluationId: number) {
    await this.assertCanSubmitEvaluationData(user, evaluationId);
  ...
  async endSession(user: JwtUser, evaluationId: number, notes?: string) {
    await this.assertCanSubmitEvaluationData(user, evaluationId);
    ...
    if (!session) throw new BadRequestException('No hay una sesión activa');
  ...
  private async computeMetrics(
    ...
    const questionEffectiveness = clamp01(
      (agg.get(UsabilityDimension.EFFECTIVENESS)?.sum ?? 0) /
        (agg.get(UsabilityDimension.EFFECTIVENESS)?.weightSum ?? 1),
    );
    const questionEfficiency = clamp01(
      (agg.get(UsabilityDimension.EFFICIENCY)?.sum ?? 0) /
        (agg.get(UsabilityDimension.EFFICIENCY)?.weightSum ?? 1),
    );
    const satisfactionScore = clamp01(
  ...
  async getResultsBreakdown(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
    const meta = await this.prisma.evaluation.findUnique({
      where: { id: evaluationId },
      select: {
    ...
    const evaluatorIds = new Set<number>();
    if (canSeeAll) {
      ...
      for (const x of assigned) evaluatorIds.add(x.evaluatorId);
      for (const x of answerDistinct) evaluatorIds.add(x.evaluatorId);
      for (const x of attemptDistinct) evaluatorIds.add(x.evaluatorId);
    } else {
    ...
    const aggregate = await this.computeMetrics(
      evaluationId,
      undefined,
      scoringWeights,
    );
  ...
  async getInterfaceBreakdown(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
    const meta = await this.prisma.evaluation.findUnique({
      where: { id: evaluationId },
      select: {
    ...
    const selectionCountByInterface = new Map<number, number>();
    for (const s of selections) {
      selectionCountByInterface.set(
        s.interfaceId,
        (selectionCountByInterface.get(s.interfaceId) ?? 0) + 1,
      );
    }
  ...
  async getCompleteness(user: JwtUser, evaluationId: number) {
    await this.assertCanViewEvaluation(user, evaluationId);
    const meta = await this.prisma.evaluation.findUnique({
      where: { id: evaluationId },
      select: { createdById: true },
    ...
    if (canSeeAll) {
      ...
      for (const x of assigned) evaluatorIds.add(x.evaluatorId);
      for (const x of answerDistinct) evaluatorIds.add(x.evaluatorId);
      for (const x of selectionDistinct) evaluatorIds.add(x.evaluatorId);
    } else {
      evaluatorIds.add(user.userId);
    }

    const ids = Array.from(evaluatorIds);
    const [users, interfaces, selectionRows, answerRows, totalAnswers] =
      await Promise.all([
    ...
    const userById = new Map(users.map((u) => [u.id, u]));
    const answerKey = (evaluatorId: number, questionId: number) =>
      `${evaluatorId}:${questionId}`;
    const answered = new Set(
      answerRows.map((r) => answerKey(r.evaluatorId, r.questionId)),
    );

    const requiredQuestionsByInterfaceId = new Map<
      number,
      Array<{ questionId: number; prompt: string }>
    >();
    for (const intf of interfaces) {
      requiredQuestionsByInterfaceId.set(
        intf.id,
        intf.questions
          .filter((q) => q.isRequired)
    ...
    const selectionByEvaluatorId = new Map(
      selectionRows.map((s) => [s.evaluatorId, s.interfaceId]),
    );
    ...
    const hasPending = evaluators.some(
      (e) => e.selection === null || e.missingRequiredAnswersCount > 0,
    );
  ...
  async listAudit(user: JwtUser, evaluationId: number) {
    await this.assertCanManageEvaluation(user, evaluationId);
